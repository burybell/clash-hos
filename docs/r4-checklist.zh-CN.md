...
### R4.3 实现状态（2026-09-10）

- 已实现蜂窝、Wi-Fi 和以太网物理承载监听，按首选网络去重并进行 2 秒防抖。
- 已增加受保护 VPN 进程的默认出口网络轮询，兼容不可靠上报承载回调的设备。
- 自动恢复限制为滚动 2 分钟内最多 3 次；超过上限会关闭隧道并写入界面可见的最终失败状态。
- 原生内核意外退出时进入同一套有限次数恢复流程。
- 配置和元数据改用“临时文件落盘后原子重命名”；活动配置更新或配置切换后无法建立已验证出口时，自动恢复内存中的上一版本快照。
- 手机锁屏/后台 20 秒冒烟测试中，界面与 VPN 进程持续存活，连接会话计时保持连续；仍需单独验证系统主动回收界面进程的场景。
- 手机网络承载切换期间 VPN 始终存在，但由于关闭 Wi-Fi 会同时断开无线 HDC，回调与轮询恢复行为仍需通过有线调试完成可重复验收。
